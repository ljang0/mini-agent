...
from mini_agent.environments.bash import BashEnvironment, SWEPatchState
from mini_agent.runtimes.base import ProcessResult
from mini_agent.runtimes.local import LocalProcessRunner, LocalRuntime
...
class SWEEnvironmentTests(unittest.IsolatedAsyncioTestCase):
    async def test_local_swe_uses_the_non_login_upstream_shell_contract(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            runner = Runner()
            environment = BashEnvironment.local(Path(temporary), runner=runner)
            try:
                await environment.execute(
    ...
    async def test_isolated_patch_export_and_transactional_adoption(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            try:
                ...
                self.assertEqual(
                    (second.runtime.workspace / "value.txt").read_text(), "changed\n"
                )
    ...
    async def test_adoption_rolls_back_after_reset_exception(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            environment = await BashEnvironment.isolated(source)
            try:
                (environment.runtime.workspace / "value.txt").write_text(
                    "prior\n", encoding="utf-8"
                )
                original_reset = environment._reset
                calls = 0
                ...
                with patch.object(
                    environment, "_reset", failing_reset_once
                ):
                ...
                self.assertEqual(
                    (environment.runtime.workspace / "value.txt").read_text(), "prior\n"
                )
            finally:
                await environment.close()
    ...
    def test_external_callers_cannot_claim_recursive_cleanup_ownership(self) -> None:
        with tempfile.TemporaryDirectory(prefix="mini-agent-swe-") as temporary:
            ...
            with self.assertRaisesRegex(ValueError, "created only by isolated"):
                LocalRuntime(workspace, owned_root=root, home=home)
