...
The 18-lab revision/card inventory is machine-readable in
[`frontier_manifest.py`](../src/scaffoldlab/applications/frontier_manifest.py) and via
`scaffoldlab list-frontier-sources --json`. It records source even when Scaffold Lab
does not execute it and reports status/implementation IDs separately for browser,
computer-use, and SWE; `catalog_gap` and `model_only` are not implementation aliases.
...
| [Browser-Use parallel template](https://docs.browser-use.com/open-source/examples/templates/parallel-browser), [0.13.7 source](https://github.com/browser-use/browser-use/tree/f0aa3a8bb03779c71a5aa262d389e3bfe6b77cdc) | Different tasks run in separate browser profiles with flat async fan-out and no manager, communication, aggregation, or selector. The repository separately publishes the full `Agent`, `Browser`, provider wrappers, prompts, history, and actions. MIT. | `flat_parallel` reproduces only the scheduling pattern and must never be labelled best-of-N. `browser_use_upstream` separately executes a bounded private archive of the pinned commit and records/checks Git and Python identities. Dependency/browser/model identity and complete accounting remain caller-controlled. |
| [OpenAI Codex rust-v0.147.0 source](https://github.com/openai/codex/tree/be6e8eac029b183056b7e4402879f15d2c85f61b), [Responses multi-agent guide](https://developers.openai.com/api/docs/guides/responses-multi-agent) | The public Rust runtime implements `codex exec`, prompts, tools, compaction, and native collaboration actions. Hosted Responses exposes a separate managed scheduler. | `codex_source` builds only a safe private archive of the pinned commit, verifies source/lock/toolchain and local build identities during manifest preflight, strips inherited Git state, requires observed completed `spawn_agent` events before calling a trial multi-agent, and exports a bounded SWE patch. Remote model catalog/snapshot, service routing, cloud/managed policy, and complete tree accounting are not pinned. |
| [Claude Code Agent Teams](https://code.claude.com/docs/en/agent-teams), [tool reference](https://code.claude.com/docs/en/tools-reference) | Claude Code publishes the Agent Teams activation flag, native team/task/config mechanics, and noninteractive CLI, but not the executable implementation source. Since v2.1.178 the team is implicit and session-derived; TeamCreate/TeamDelete no longer exist and `Agent.team_name` is ignored. | `claude_code_agent_teams_distribution` verifies the audited official 2.1.226 Darwin arm64 executable/package bytes, requires distinct named Agent calls, snapshots the matching live session roster before automatic cleanup, corroborates the persistent session task directory, uses fresh config/Git state, and exports a patch. It is a distribution adapter, not source or system-card parity; server-managed policy and whole-team accounting remain unobservable. |
...
| [Prime Agent launch](https://www.primeintellect.ai/blog/prime-agent), [multi-agent systems analysis](https://www.primeintellect.ai/blog/multi-agent-systems), [0.7.1 release](https://github.com/PrimeIntellect-ai/prime-agent/releases/tag/v0.7.1), [tagged source](https://github.com/PrimeIntellect-ai/prime-agent/tree/95afd319a78ae017a41241d50b013d656a0685ce), [JSON v3 contract](https://github.com/PrimeIntellect-ai/prime-agent/blob/95afd319a78ae017a41241d50b013d656a0685ce/packages/coding-agent/docs/json.md) | Persistent IPython parent; asynchronous full child sessions; explicit messages/files; daemon persistence, compaction, goals, schedules, bounded autonomous mode, and continual harness state. The release tag resolves to commit `95afd31`; the exact `package-lock.json` is also audited. MIT. | `prime_agent` preserves the installed CLI's exact JSON v3 boundary. `prime_agent_source` is a separate caller-built-runtime study: it executes a private immutable copy of caller-provided bundle bytes beside pinned source/lockfile evidence, but no adapter-owned build or authoritative released digest proves the source-to-bundle relationship. Both use fresh `--no-session` state; cross-invocation state and complete child-tree accounting remain absent. |
| [Grok Build source](https://github.com/xai-org/grok-build/tree/8a14c91d88875a831a38b3a066b1683116bcb31c), [pinned headless contract](https://github.com/xai-org/grok-build/blob/8a14c91d88875a831a38b3a066b1683116bcb31c/crates/codegen/xai-grok-pager/docs/user-guide/14-headless-mode.md), [npm 1.0.0](https://www.npmjs.com/package/@xai-official/grok/v/1.0.0), [sandbox docs](https://docs.x.ai/build/features/sandbox) | Released coding harness with native subagents, tools, permissions, sandbox profiles, worktrees, compaction, and a single-object headless JSON result. The npm `gitHead`, public-repository commit, and repository `SOURCE_REV` differ, so one generic “1.0.0” label is insufficient identity. | `grok_build` preserves the installed CLI protocol. `grok_build_source` exports the pinned revision into a private build root, rejects external Cargo configuration, enforces `SOURCE_REV` and Cargo.lock, records concrete Git/Cargo/rustc/binary identities, executes native subagents in a disposable Git baseline, and exports a patch. Optional SHA pins are required for comparable local build bytes. Both retain terminal usage as an incomplete lower bound. |
...
- **`upstream_runtime_adapter`:** a pinned public source artifact, or a digest-checked
  official distribution when matching source is unavailable, owns the runtime;
  Scaffold Lab records isolation, observed build/executable identity, artifacts, and
  only the accounting exposed by that runtime. This label alone is not a
  bit-reproducibility claim.
- **`caller_built_runtime_study`:** a caller-built artifact runs beside pinned
  source and lockfile evidence, but no authoritative build or released digest proves
  that the runtime bytes were generated from that source.
...
`application.implementation` selector. The five study labels are
...
| --- | --- | --- |
| `browser` | OpenAI hosted multi-agent with client functions or hosted `web_search`; xAI 4/16-agent hosted web research; Anthropic Managed Agents; pinned Browser-Use 0.13.7 and MACU runtimes | Browser-Use flat parallel pattern, local MACU, and Mythos/Opus topology studies; Meta Muse scheduler and BrowserGym evaluator gaps |
...
| `swe` | Anthropic Managed Agents; official Claude Code Agent Teams distribution; pinned Codex, Grok, Kimi Code, and RLM runtimes; installed Prime/Grok protocol profiles | Caller-built Prime source bundle, local controls, composed OpenAI protocols, restricted RLM, Platoon inference, MACU, and card-topology studies; RAO training, Meta Muse scheduling, xAI developer tools, and SWE-bench evaluator gaps |

## Frontier-lab disclosures

| Lab | Strongest reproducible public evidence | Classification |
| --- | --- | --- |
| OpenAI | [GPT-5.6 Responses multi-agent guide](https://developers.openai.com/api/docs/guides/responses-multi-agent) publishes the hosted invocation, six coordination actions, injected prompts, context inheritance, independent compaction, recommended concurrency, and stateless HTTP/WebSocket continuation. The [GPT-5.6 system card](https://deploymentsafety.openai.com/gpt-5-6) is separate model/safety evidence. The [GA computer guide](https://developers.openai.com/api/docs/guides/tools-computer-use), [shell guide](https://developers.openai.com/api/docs/guides/tools-shell), and [pinned Codex source](https://github.com/openai/codex/tree/be6e8eac029b183056b7e4402879f15d2c85f61b) expose distinct public boundaries. | Browser and computer-use execute the published hosted/client protocols. SWE additionally executes pinned Codex 0.147.0 source with native collaboration and patch capture. The hosted scheduler, WebSocket path, remote model catalog/snapshot, private routing, cloud/managed policy, and complete tree accounting remain outside the claim; composed hosted SWE protocols remain studies. |
| Anthropic | The [joint Fable/Mythos 5 system card](https://www-cdn.anthropic.com/2f9323abbcc4abe219577539efe19a623c9ca2bd/Claude%20Fable%205%20%26%20Claude%20Mythos%205%20System%20Card.pdf) §8.15.3 evaluates **Mythos 5** with blocking, fixed-team, and async harnesses. The [Opus 5 system card](https://www-cdn.anthropic.com/b514064af1408018e64b1ad24e7d5e75850b4ffd/Claude%20Opus%205%20System%20Card.pdf) §8.11.3 specifies fixed N-agent and async harnesses. [Claude Code Agent Teams](https://code.claude.com/docs/en/agent-teams) publishes a separate product/distribution interface. Managed Agents and Messages computer-use publish still other hosted/client protocols. | System-card variants remain topology simulations because exact prompts, timing, evaluator state, per-agent limits, compaction, safeguards, and worktree mechanics are not all public. SWE separately executes the audited official Claude Code 2.1.226 Darwin arm64 distribution and verifies native team/config evidence, but not matching source or server-managed policy. Browser Managed Agents and computer-use execute their own published protocols; none is relabelled as the card harness. |
| xAI | The [pinned Grok Build snapshot](https://github.com/xai-org/grok-build/tree/8a14c91d88875a831a38b3a066b1683116bcb31c) releases subagent source and worktree/capability semantics. Its [current headless interface](https://docs.x.ai/build/cli/headless-scripting) supports scriptable, machine-readable external invocation. The [Grok multi-agent API](https://docs.x.ai/developers/model-capabilities/text/multi-agent) exposes a team total of 4 agents for low/medium effort or 16 for high/xhigh, including a designated leader and hosted `web_search`/`x_search` tools; the [Grok 4.20 card](https://data.x.ai/2026-04-07-grok-4-20-model-card.pdf) is separate model evidence. | `grok_build` implements the installed version-1.0.0 headless CLI boundary. `grok_build_source` runs the audited public source with its internal revision/lock checks. `xai_hosted_multi_agent` pins model snapshot `grok-4.20-multi-agent-0309` and reproduces the documented 4/16-agent HTTP boundary and server-tool declarations. Compiler/package identity, plaintext child state, complete accounting, encrypted continuation, and hosted scheduler internals remain outside their respective scopes. |
...
RLM release ordering cannot be inferred from semantic versions. The `v1.0.0` Git tag
was published in January 2026 and is an ancestor of the June 2026 `v0.1.3` release,
which GitHub marks latest after an upstream package/version reset. The older API also
lacks the current budget, recursion, backend, and answer-dictionary contract used by
this adapter, so replacing 0.1.3 with 1.0.0 would be a regression rather than an
upgrade.

Prime's installed protocol and caller-built study, Grok, Browser-Use, Kimi, Codex,
and the Claude Code distribution profiles all
make one outer shared-ledger call for the complete upstream-owned tree. Source
adapters record their pinned revision boundary; Claude records official package and
executable identities instead of pretending its public tag is runtime source. This
preserves Scaffold Lab's trace/budget contract without injecting local tools into
those runtimes. It does not create equal-compute evidence: every listed adapter lacks
independently verified complete inner-tree accounting. No paid run has been used to
promote any of them as a release winner.
...
| [Browser-Use 0.13.7](https://github.com/browser-use/browser-use/tree/f0aa3a8bb03779c71a5aa262d389e3bfe6b77cdc) | Browser agent runtime and examples. | `browser_use_upstream` executes the pinned policy/browser runtime. Its flat-parallel example separately informs `flat_parallel`; local tools do not reproduce the policy. |
